#!/usr/bin/env npx tsx
/**
 * Run WP-242 tail backfill (Python) with KNOWLEDGE_DATABASE_URL from .dev.vars
 * One-shot script, not meant for production use.
 * Usage: npx tsx scripts/run-wp242-tail-backfill.ts [--dry-run]
 */
...
const backfillScript = join(__dirname, "wp242-tail-backfill.py");

const args = process.argv.slice(2); // forward --dry-run
