    ...
    try {
      const llmResponse = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.1,
          // 500 -> 1500: ответ теперь несёт quotes (дословные цитаты) вдобавок
          // к explanations - при нескольких находках старый лимит рисковал
          // обрезать JSON и уводить в substring-фолбэк (WP-7, code review).
          max_tokens: 1500,
          response_format: { type: "json_object" },
        }),
      });
