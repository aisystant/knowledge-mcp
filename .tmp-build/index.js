...
var VECTOR_CONFIDENCE_THRESHOLD = 0.6;
var CHUNK_CHAR_LIMIT = 1e4;
var LARGE_FILE_THRESHOLD = CHUNK_CHAR_LIMIT;
var MAX_FILE_SIZE = 1e5;
var RERANK_MODEL = "gpt-4o-mini";
var RERANK_CANDIDATES = 20;
var RERANK_TIMEOUT_MS = 5e3;
var RERANK_VECTOR_WEIGHT = 0.3;
var RERANK_LLM_WEIGHT = 0.7;
var SOURCE_GITHUB_BASE = {
...
function resolveGithubUrl(source, filename) {
...
__name(resolveGithubUrl, "resolveGithubUrl");
...
function db(env) {
  return cs(env.DATABASE_URL);
}
__name(db, "db");
function detectQueryType(query) {
  if (/[A-Z]{2,}\.\w+\.\d+/.test(query))
    return "keyword";
  if (query.length < 30 && /\.[A-Z]/.test(query))
    return "keyword";
  return "vector";
}
__name(detectQueryType, "detectQueryType");
async function keywordSearch(env, query, source, sourceType, limit = 5) {
  const sql = db(env);
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      score: r.score,
      github_url: resolveGithubUrl(source2, filename)
    };
  });
}
__name(keywordSearch, "keywordSearch");
async function vectorSearch(env, query, source, sourceType, limit = 5) {
...
async function rerankWithLLM(apiKey, query, results, limit) {
  if (results.length <= 1)
...
${c.snippet}`).join("\n\n")}`;
  try {
    ...
    const scores = Array.isArray(parsed) ? parsed : Array.isArray(parsed.scores) ? parsed.scores : [];
    if (scores.length === 0)
      return results.slice(0, limit);
    const llmScoreMap = /* @__PURE__ */ new Map();
...
async function enrichWithParentContent(env, results) {
  if (results.length === 0)
    return results;
  const sql = db(env);
...
__name(enrichWithParentContent, "enrichWithParentContent");
async function searchDocuments(env, query, source, sourceType, limit = 5) {
  const queryType = detectQueryType(query);
  if (queryType === "keyword") {
    const kwResults = await keywordSearch(env, query, source, sourceType, limit);
    if (kwResults.length > 0)
      return enrichWithParentContent(env, kwResults);
  }
  ...
  if (vectorResults.length > 0 && vectorResults[0].score < VECTOR_CONFIDENCE_THRESHOLD) {
    ...
    if (kwFallback.length > 0) {
      ...
      const merged = [...seen.values()].sort((a2, b2) => b2.score - a2.score);
      const reranked2 = await rerankWithLLM(env.OPENAI_API_KEY, query, merged, limit);
      return enrichWithParentContent(env, reranked2);
    }
  }
...
async function getDocument(env, filename, source) {
  const sql = db(env);
  const src = source ?? null;
  const rows = await sql`
  ...
  if (!rows.length)
    return null;
  const r = rows[0];
  const docSource = r.source || "";
  const docFilename = r.filename;
  return {
...
async function listSources(env, sourceType) {
  const sql = db(env);
  const stype = sourceType ?? null;
  const rows = await sql`
...
async function recordFeedback(env, documentId, query, helpfulness) {
  const sql = db(env);
...
async function hashQuery(query) {
  ...
  return Array.from(new Uint8Array(hash)).map((b2) => b2.toString(16).padStart(2, "0")).join("").slice(0, 64);
}
__name(hashQuery, "hashQuery");
async function getFeedbackStats(env, days = 30, limit = 20) {
  const sql = db(env);
  const rows = await sql`
    SELECT
      f.document_id,
...
__name(getFeedbackStats, "getFeedbackStats");
async function analyzeVerbalization(env, text, topic, domain, level, userId) {
  const sql = db(env);
  let topicConcepts;
  if (topic) {
...
\u0415\u0441\u043B\u0438 \u043D\u0438\u0447\u0435\u0433\u043E \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D\u043E: {"found": [], "confidence": []}`;
  ...
  matched.sort((a2, b2) => b2.score - a2.score);
  missed.sort((a2, b2) => b2.centrality - a2.centrality);
  const matchedIds = matched.map((m2) => topicConcepts.find((c) => c.code === m2.code)?.id).filter(Boolean);
  let edgeCoverage = 0;
  ...
  const misconceptions = await sql`
    ...
    LIMIT 30
  `;
  const foundMisconceptions = [];
  if (misconceptions.length > 0 && text.length > 20) {
    const miscCatalog = misconceptions.map(
      (m2, i) => `[${i}] \u041C\u0435\u043C: ${m2.misconception_text} | \u041F\u043E\u043D\u044F\u0442\u0438\u0435: ${m2.concept_name} | \u0422\u0438\u043F: ${m2.category}${m2.correct_version ? ` | \u041F\u0440\u0430\u0432\u0438\u043B\u044C\u043D\u043E: ${m2.correct_version}` : ""}${m2.folk_term ? ` | \u0411\u044B\u0442\u043E\u0432\u043E\u0439 \u0442\u0435\u0440\u043C\u0438\u043D: ${m2.folk_term}` : ""}`
    ).join("\n");
...
\u0415\u0441\u043B\u0438 \u043C\u0435\u043C\u043E\u0432 \u043D\u0435 \u043E\u0431\u043D\u0430\u0440\u0443\u0436\u0435\u043D\u043E: {"found": [], "explanations": []}`;
      ...
      if (llmResponse.ok) {
        ...
        for (let i = 0; i < (parsed.found || []).length; i++) {
          ...
          if (idx >= 0 && idx < misconceptions.length) {
            const m2 = misconceptions[idx];
            foundMisconceptions.push({
              text: m2.misconception_text,
              correct: m2.correct_version,
              category: m2.category,
              explanation: parsed.explanations?.[i] || void 0
            });
          }
        }
      }
    } catch {
      for (const m2 of misconceptions) {
        const miscText = m2.misconception_text.toLowerCase().replace(/[«»"]/g, "");
  ...
  for (const m2 of topMissed) {
    recommendations.push(`\u041F\u0440\u043E\u0433\u043E\u0432\u043E\u0440\u0438\u0442\u0435 \xAB${m2.name}\xBB \u2014 \u0432\u0430\u0436\u043D\u043E\u0435 \u043F\u043E\u043D\u044F\u0442\u0438\u0435 \u0442\u0435\u043C\u044B.`);
  }
  ...
  return {
    ...
    edge_coverage: Math.round(edgeCoverage * 1e3) / 1e3,
    misconceptions_found: foundMisconceptions,
    recommendations,
    mastery_updated: !!userId
  };
}
__name(analyzeVerbalization, "analyzeVerbalization");
async function getGraphStats(env) {
  const sql = db(env);
...
__name(getGraphStats, "getGraphStats");
async function getLearnerProgress(env, userId, domain) {
  const sql = db(env);
  ...
  return {
    ...
    coverage: totalConcepts.cnt > 0 ? Math.round(masteredCount.cnt / totalConcepts.cnt * 1e3) / 1e3 : 0,
...
var TOOLS = [
  ...
  },
  {
    ...
    description: "Record helpfulness feedback for a retrieved document. Call after using a search result to signal whether it was actually helpful. This data improves future retrieval quality.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "feedback_stats",
    description: "Analytics: top documents by helpfulness feedback over a period. Shows which documents are actually used by agents.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "\u0410\u043D\u0430\u043B\u0438\u0437\u0438\u0440\u0443\u0435\u0442 \u0442\u0435\u043A\u0441\u0442 \u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044F (\u0414\u0417, \u044D\u0441\u0441\u0435, \u043F\u0440\u043E\u0433\u043E\u0432\u0430\u0440\u0438\u0432\u0430\u043D\u0438\u0435) \u043D\u0430 \u043F\u0440\u0435\u0434\u043C\u0435\u0442 \u0438\u0441\u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u043D\u0438\u044F \u043F\u043E\u043D\u044F\u0442\u0438\u0439 \u0438\u0437 \u0433\u0440\u0430\u0444\u0430 \u0437\u043D\u0430\u043D\u0438\u0439. \u0412\u043E\u0437\u0432\u0440\u0430\u0449\u0430\u0435\u0442: \u043F\u043E\u043A\u0440\u044B\u0442\u0438\u0435 \u043F\u043E\u043D\u044F\u0442\u0438\u0439, \u043F\u0440\u043E\u043F\u0443\u0449\u0435\u043D\u043D\u044B\u0435 \u043A\u043B\u044E\u0447\u0435\u0432\u044B\u0435 \u043F\u043E\u043D\u044F\u0442\u0438\u044F, \u0441\u0432\u044F\u0437\u043D\u043E\u0441\u0442\u044C, \u043E\u0431\u043D\u0430\u0440\u0443\u0436\u0435\u043D\u043D\u044B\u0435 \u043C\u0435\u043C\u044B/\u043F\u043E\u0434\u043C\u0435\u043D\u044B, \u0440\u0435\u043A\u043E\u043C\u0435\u043D\u0434\u0430\u0446\u0438\u0438. \u0418\u0441\u043F\u043E\u043B\u044C\u0437\u0443\u0439 \u0434\u043B\u044F \u043E\u0446\u0435\u043D\u043A\u0438 \u043A\u0430\u0447\u0435\u0441\u0442\u0432\u0430 \u043F\u0440\u043E\u0433\u043E\u0432\u0430\u0440\u0438\u0432\u0430\u043D\u0438\u044F \u0438\u043B\u0438 \u043F\u0438\u0441\u044C\u043C\u0435\u043D\u043D\u043E\u0439 \u0440\u0430\u0431\u043E\u0442\u044B.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "graph_stats",
    description: "\u0421\u0442\u0430\u0442\u0438\u0441\u0442\u0438\u043A\u0430 \u0433\u0440\u0430\u0444\u0430 \u043F\u043E\u043D\u044F\u0442\u0438\u0439: \u043A\u043E\u043B\u0438\u0447\u0435\u0441\u0442\u0432\u043E \u043F\u043E\u043D\u044F\u0442\u0438\u0439 \u043F\u043E \u0443\u0440\u043E\u0432\u043D\u044F\u043C, \u0441\u0432\u044F\u0437\u0438 \u043F\u043E \u0442\u0438\u043F\u0430\u043C, orphan-\u043F\u043E\u043D\u044F\u0442\u0438\u044F (\u0431\u0435\u0437 \u0441\u0432\u044F\u0437\u0435\u0439), \u043F\u043E\u0434\u043E\u0437\u0440\u0438\u0442\u0435\u043B\u044C\u043D\u044B\u0435 edges \u0434\u043B\u044F \u0440\u0435\u0432\u044C\u044E, \u0442\u043E\u043F-10 \u0446\u0435\u043D\u0442\u0440\u0430\u043B\u044C\u043D\u044B\u0445 \u043F\u043E\u043D\u044F\u0442\u0438\u0439.",
    inputSchema: {
      type: "object",
      properties: {}
    }
  },
  {
    ...
    description: "\u041F\u0440\u043E\u0433\u0440\u0435\u0441\u0441 \u0443\u0447\u0435\u043D\u0438\u043A\u0430 \u043F\u043E \u043E\u0441\u0432\u043E\u0435\u043D\u0438\u044E \u043F\u043E\u043D\u044F\u0442\u0438\u0439: \u043E\u0431\u0449\u0435\u0435 \u043F\u043E\u043A\u0440\u044B\u0442\u0438\u0435, \u0440\u0430\u0437\u0431\u0438\u0432\u043A\u0430 \u043F\u043E \u0434\u043E\u043C\u0435\u043D\u0430\u043C, \u0442\u043E\u043F \u043E\u0441\u0432\u043E\u0435\u043D\u043D\u044B\u0445, \u0441\u043B\u0430\u0431\u044B\u0435 \u043C\u0435\u0441\u0442\u0430, \u0440\u0435\u043A\u043E\u043C\u0435\u043D\u0434\u0430\u0446\u0438\u0438 \u0447\u0442\u043E \u0443\u0447\u0438\u0442\u044C \u0434\u0430\u043B\u044C\u0448\u0435. \u0414\u0430\u043D\u043D\u044B\u0435 \u043E\u0431\u043D\u043E\u0432\u043B\u044F\u044E\u0442\u0441\u044F \u043F\u0440\u0438 \u043A\u0430\u0436\u0434\u043E\u043C \u0432\u044B\u0437\u043E\u0432\u0435 analyze_verbalization \u0441 user_id.",
    inputSchema: {
      type: "object",
      properties: {
...
async function handleMcpRequest(request, env) {
  ...
  try {
    switch (method) {
      ...
      case "tools/call": {
        ...
        if (toolName === "get_document") {
          const doc = await getDocument(env, args.filename, args.source);
          if (!doc) {
            return {
              jsonrpc: "2.0",
              id,
        ...
        if (toolName === "list_sources") {
          const sources = await listSources(env, args.source_type);
          return {
            jsonrpc: "2.0",
            id,
        ...
        if (toolName === "feedback_stats") {
          const stats = await getFeedbackStats(env, args.days || 30, args.limit || 20);
          return {
            jsonrpc: "2.0",
            id,
    ...
    return {
      jsonrpc: "2.0",
      id,
      error: { code: -32e3, message: err instanceof Error ? err.message : "Unknown error" }
    };
  }
}
...
async function contentHash(content) {
  ...
  return Array.from(new Uint8Array(hashBuffer)).map((b2) => b2.toString(16).padStart(2, "0")).join("").slice(0, 16);
}
__name(contentHash, "contentHash");
async function readFromGitHubPublic(source, filePath) {
  const config = SOURCE_GITHUB_BASE[source];
  if (!config)
    return null;
  ...
  const fullPath = config.pathPrefix ? `${config.pathPrefix}${filePath}` : filePath;
...
__name(readFromGitHubPublic, "readFromGitHubPublic");
function resolveSourceType(source) {
  if (source.startsWith("PACK-"))
    return "pack";
  if (source.startsWith("DS-"))
    return "ds";
  if (source === "SPF" || source === "FPF" || source === "ZP")
    return "pack";
  if (source.startsWith("docs-") || source.endsWith("-docs"))
    return "guides";
  if (source.startsWith("FMT-"))
    return "ds";
  return "ds";
}
__name(resolveSourceType, "resolveSourceType");
function chunkLargeFile(content, filename) {
  const chunks = [];
  ...
  for (const section of sections) {
    ...
    if (section.length <= CHUNK_CHAR_LIMIT) {
      const prefix = docTitle ? `> [${filename.split("::")[0]}] ${docTitle} > ${sectionName}
...
` : "";
      ...
      for (const subsection of subsections) {
        ...
        if (subsection.length <= CHUNK_CHAR_LIMIT) {
          const prefix = docTitle ? `> [${filename.split("::")[0]}] ${docTitle} > ${subName}
...
__name(chunkLargeFile, "chunkLargeFile");
async function reindexFiles(env, req) {
  const sql = db(env);
  const result = { processed: 0, deleted: 0, skipped: 0, errors: [] };
  ...
  for (const file of req.files) {
    ...
    try {
      if (file.action === "removed") {
        await sql`DELETE FROM documents WHERE source = ${req.source} AND (filename = ${file.path} OR filename LIKE ${file.path + "::%"})`;
        result.deleted++;
        continue;
      }
      ...
      const existing = await sql`SELECT hash FROM documents WHERE filename = ${file.path} AND source = ${req.source} LIMIT 1`;
      ...
      if (content.length > LARGE_FILE_THRESHOLD) {
        ...
        await sql`
          ...
          VALUES (${file.path}, ${content}, ${req.source}, ${sourceType}, ${hash}, NULL, to_tsvector('simple', ${content}))
        ...
        for (const chunk of chunks) {
          ...
          await sql`
            ...
            VALUES (
              ...
              (SELECT id FROM documents WHERE filename = ${file.path} AND source = ${req.source} LIMIT 1)
            )
            ON CONFLICT (filename, source)
            DO UPDATE SET
              ...
              parent_id = (SELECT id FROM documents WHERE filename = ${file.path} AND source = ${req.source} LIMIT 1)
          `;
        }
      } else {
        ...
        await sql`
          INSERT INTO documents (filename, content, source, source_type, hash, embedding, search_vector)
          VALUES (${file.path}, ${content}, ${req.source}, ${sourceType}, ${hash}, ${vec}::vector, to_tsvector('simple', ${content}))
          ON CONFLICT (filename, source)
...
__name(reindexFiles, "reindexFiles");
var src_default = {
